...
let RentRollPage = ({
    ...
    const getMappedStatements = () => {
        const mappedTransactions = transactions.filter((transaction) => moment(transaction.transaction_date).month() === moment().month()).map((transaction) => {
            ...
            transactionDetails.days_left = moment(transaction.lease_end).diff(moment(transaction.lease_start), 'days')
            if (typeof tenant !== 'undefined') {
                transactionDetails.tenant = tenant.first_name + ' ' + tenant.last_name

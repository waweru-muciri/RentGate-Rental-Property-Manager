...
const contactsTableHeadCells = [
    ...
    const rows = [
		{ id: 1, user_email: 'user@yourdomain.com', action: "create", entity: 'property', user: "user001", log_date: '12/12/2012',  entity_id: '5ee61c19ed13fc00027af18b', },
		{ id: 2, user_email: 'user@yourdomain.com', action: "create", entity: 'property', user: "user002",  log_date: '12/12/2012',  entity_id: '5ee61c19ed13fc00027af18b', },
		{ id: 3, user_email: 'user@yourdomain.com', action: "create", entity: 'property', user: "user003",  log_date: '12/12/2012',  entity_id: '5ee61c19ed13fc00027af18b', }, 
	]
...
let AuditLogsPage = ({
    ...
    const exportContactRecordsToExcel = () => {
        ...
        exportDataToXSL('Audit Logs  Records', 'Contact Data', items, 'ContactData')
    }
        ...
        <Layout pageTitle="Audit Logs">
            <Grid container spacing={3} justify="flex-start"
				...
				<Box border={1} borderRadius="borderRadius" borderColor="grey.400">
            		<form className={classes.form} id="logSearchForm" onSubmit={ handleSearchFormSubmit}>
					<Grid container spacing={2} justify="center" direction="row">
						...
						<Grid item lg={6} md={12} xs={12}>
							<TextField fullWidth select variant="outlined" id="user" name="user" label="User" value={userFilter} onChange={(event) => {setUserFilter(event.target.value)}} > 
							{
						...
						<Grid item lg={6} md={12} xs={12}>
							<TextField fullWidth  variant="outlined" id="user_email" name="user_email" label="User Email" value={firstNameFilter || ''} onChange={(event) => {setUserEmailFilter(event.target.value)}}  />
						</Grid>
						<Grid item lg={6} md={12} xs={12}>
						<TextField fullWidth variant="outlined" name="action" label="Action" id="action"  onChange={(event) => {setActionFilter(event.target.value)}} value={actionFilter || ''} />
						</Grid>
						<Grid item lg={6} md={12} xs={12}>
						<TextField fullWidth select variant="outlined" name="entities_ids" label="Entities" id="entities_ids"  onChange={(event) => {setEntitiesIdsFilter(event.target.value)}} value={entitiesIdsFilter} >
			{
									ENTITIES_LIST.map((entity, index) => <MenuItem key={index} value={entity}>{entity}</MenuItem>)
								}
						</TextField>
						</Grid >
					</Grid> 
				<Grid container spacing={2} item justify="flex-end" alignItems="center"  direction="row" key={1}>
					<Grid item>
								<Button type="submit" form="logSearchForm" color="primary" variant="contained" size='medium'startIcon={<SearchIcon />} >SEARCH </Button>
					</Grid> 
					<Grid item>
								<Button onClick={event => { resetSearchForm(event) }} type="reset" form="logSearchForm" color="primary" variant="contained" size='medium' startIcon={<UndoIcon />} >RESET </Button>
					</Grid>
				</Grid>
					</form>
				</Box>
                ...
                <Grid item lg={12} md={12} sm={12} xl={12} xs={12}>
                    ...
                    <CommonTable  selected={selected} setSelected={setSelected} rows={contactItems} headCells={contactsTableHeadCells} />
                </Grid>
			{

...
const usersTableHeadCells = [
    ...
    const rows = [
		{ id: 1, first_name: 'Brian', last_name: "Muciri", status: 'active', email: "bwwaweru18@gmail.com", date_created: '12/12/2012',  phone_number: '254742654637', },
		{ id: 2, first_name: 'Brian', last_name: "Muciri", status: 'active', email: "bwwaweru18@gmail.com",  date_created: '12/12/2012',  phone_number: '254742654637', },
		{ id: 3, first_name: 'Brian', last_name: "Muciri", status: 'active', email: "bwwaweru18@gmail.com",  date_created: '12/12/2012',  phone_number: '254742654637', }, 
	]
        ...
        <Layout pageTitle="Users">
            <Grid container spacing={3} justify="space-evenly"
				...
				<Grid container spacing={2} item  alignItems="center"  direction="row" key={1}>
					...
					<Grid item>
								<Button type="button" color="primary" variant="contained" size='medium'startIcon={<EditIcon />} disabled={ selected.length <= 0 } component={Link} to={`${match.url}${selected[0]}/edit`}> Edit User
								</Button>
					</Grid>
					<Grid item>
                		<ExportToExcelBtn aria-label="Export to Excel" disabled={ selected.length <= 0 } onClick={event => { exportUserRecordsToExcel() }}/>
					</Grid>
				</Grid>
                <Grid item xs={12} sm={12} md={12} lg={12} >
				<Box border={1} borderRadius="borderRadius" borderColor="grey.400">
            		<form className={classes.form} id="userSearchForm" onSubmit={ handleSearchFormSubmit}>
					...
					<Grid container spacing={2}justify="center" direction="row">
						<Grid item lg={6} md={12} xs={12}>
							<TextField fullWidth select variant="outlined" id="email" name="email" label="Role" value={roleFilter || ''} onChange={(event) => {setRoleFilter(event.target.value)}} >
								{
									ROLES_LIST.map((role, index) => <MenuItem key={index} value={role}>{role}</MenuItem>)
								}
						...
						<TextField fullWidth select variant="outlined" name="status" label="Status" id="status"  onChange={(event) => {setStatusFilter(event.target.value)}} value={statusFilter || ''} >
			{
				...
				<Grid container spacing={2} item justify="flex-end" alignItems="center"  direction="row" key={1}>
					<Grid item>
								<Button type="submit" form="userSearchForm" color="primary" variant="contained" size='medium'startIcon={<SearchIcon />} >SEARCH </Button>
					</Grid> 
					<Grid item>
                ...
                <Grid item lg={12} md={12} sm={12} xl={12} xs={12}>
                    ...
                    <CommonTable  selected={selected} setSelected={setSelected} rows={userItems} headCells={usersTableHeadCells} />
                </Grid>
			{

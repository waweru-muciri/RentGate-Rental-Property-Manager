...
const db = firebase.firestore(app).collection('tenant').doc('wPEY7XfSReuoOEOa22aX');
		...
		//console.log('Items query snapshot => ', items);
            ...
            // .then((response) => {
             //   let items = response.data
                ...
                switch (url) {
                    case 'properties':
                        dispatch(propertyActions.propertiesFetchDataSuccess(items));
                        break;

                    case 'contacts':
                        dispatch(contactsActions.contactsFetchDataSuccess(items))
                        break;
                    ...
                    case 'contact_phone_numbers':
                        dispatch(phoneNumbersActions.phoneNumbersFetchDataSuccess(items))
                        break;
                    ...
                    case 'contact_addresses':
                        dispatch(addressesActions.addressesFetchDataSuccess(items))
                        break;

                    case 'transactions':
                        dispatch(transactionsActions.transactionsFetchDataSuccess(items))
                        break;
                    ...
                    case 'maintenance-requests':
                        dispatch(maintenanceRequestsActions.maintenanceRequestsFetchDataSuccess(items))
                        break;
...
export function handleDelete(itemId, url) {
    ...
    return (dispatch) => {
        dispatch(itemsIsLoading(true));
        axiosInstance.delete(`${url}/${itemId}/`)
            .then((response) => {
                dispatch(itemsIsLoading(false));
                switch (url) {
                    case 'properties':
    ...
    return (dispatch) => {
        ...
        if (typeof data.id !== 'undefined') {
            ...
            //axiosInstance.patch(`${url}/${data.id}/`, data)
				db.collection(url).doc(data.id).update(data)
                .then((response) => {
                    ...
                    case 'transactions':
                        dispatch(transactionsActions.editTransaction(savedItem))
                        break;
                    ...
                    case 'maintenance-requests':
                        dispatch(maintenanceRequestsActions.editMaintenanceRequest(savedItem))
                        break;
        ...
        //send post to create item
            ...
            //axiosInstance.post(`${url}/`, data)
                ...
                .then((response) => {
                    ...
                    case 'transactions':
                        dispatch(transactionsActions.addTransaction(savedItem))
                        break;
                    ...
                    case 'maintenance-requests':
                        dispatch(maintenanceRequestsActions.addMaintenanceRequest(savedItem))
                        break;
